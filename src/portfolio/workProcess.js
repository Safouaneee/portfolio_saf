function Work_process(){
    return(
        <section id="work_process">
            <div className="description_work">
                <div id="wk" className="welcome">
                    <p>WORK PROCESS</p>
                </div>
                <h2>
                    Work Process
                </h2>
                <p id="desc_work">A glimpse into my collaborative and iterative full-stack development process From designing intuitive interfaces to building robust and scalable backend systems.</p>
                <button>Read more</button>

            </div>
            <div className="work_pr">
                <img src={`${process.env.PUBLIC_URL}/dashed_circle.png`}></img>
                <div className="ligne1">
                        <div className="atape1">
                            <i class="ri-lightbulb-line"></i>
                            <h1>01</h1>
                            <h3>Plan & Analyze</h3>
                        </div>
                        <div className="atape2">
                            <i class="ri-pencil-ruler-2-line"></i>
                            <h1>02</h1>
                            <h3>Design & Prototype</h3>
                        </div>
                 </div>
                 <div className="ligne2">

                        <div className="etape3">
                            <i class="ri-code-line"></i>
                            <h1>03</h1>
                            <h3>Develop & Implement</h3>
                        </div>
                        <div className="etape4">
                            <i class="ri-rocket-line"></i>
                            <h1>03</h1>
                            <h3>Test & Deploy</h3>
                        </div>
                 </div>
            </div>
        </section>
    )

}export default Work_process